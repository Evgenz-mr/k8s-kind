...
if (($Ingress -ne 'none' -or $Dashboard -ne 'none' -or $ArgoCD -eq 'enabled' -or $CertManager -eq 'enabled') -and -not (Get-Command helm -ErrorAction SilentlyContinue)) {
...
if ($CertManager -eq 'enabled') { Write-Host 'cert-manager: enabled (ClusterIssuer lab-selfsigned)' }
