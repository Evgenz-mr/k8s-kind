...
$lines=@('kind: Cluster','apiVersion: kind.x-k8s.io/v1alpha4',"name: $Name");if($Network-eq'cilium'){$lines+=@('networking:','  disableDefaultCNI: true')};$lines+=@('nodes:','  - role: control-plane','    extraPortMappings:','      - containerPort: 80','        hostPort: 8080','        protocol: TCP','      - containerPort: 443','        hostPort: 8443','        protocol: TCP');for($i=1;$i-le$Workers;$i++){$lines+='  - role: worker'};$lines|Set-Content $cfg -Encoding utf8
kind create cluster --name $Name --config $cfg;if($LASTEXITCODE-ne 0){throw 'kind create cluster failed.'};$context="kind-$Name"
if($Network-eq'cilium'){helm repo add cilium https://helm.cilium.io/ --force-update;helm repo update;helm upgrade --install cilium cilium/cilium --kube-context $context -n kube-system --set ipam.mode=kubernetes --set image.pullPolicy=IfNotPresent;if($LASTEXITCODE-ne 0){throw 'Cilium installation failed.'};kubectl --context $context -n kube-system rollout status ds/cilium --timeout=300s}
