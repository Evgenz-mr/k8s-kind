...
if (($Dashboard -eq 'headlamp' -or $ArgoCD -eq 'enabled' -or $Monitoring -eq 'enabled') -and $Ingress -eq 'none') {
    throw 'Dashboard, Argo CD and Monitoring require Ingress.'
}

$dir = Join-Path $PSScriptRoot '..\generated'
New-Item -ItemType Directory -Force $dir | Out-Null
$cfg = Join-Path $dir "$Name.yaml"
$lines = @('kind: Cluster','apiVersion: kind.x-k8s.io/v1alpha4',"name: $Name")
if ($Network -eq 'cilium') { $lines += @('networking:','  disableDefaultCNI: true') }
$lines += @(
    'nodes:',
    '  - role: control-plane',
    '    extraPortMappings:',
...
if ($Network -eq 'cilium') {
    ...
    helm repo update
    helm upgrade --install cilium cilium/cilium --kube-context $context -n kube-system --set ipam.mode=kubernetes --set image.pullPolicy=IfNotPresent
    if ($LASTEXITCODE -ne 0) { throw 'Cilium installation failed.' }
...
if ($Postgres -eq 'enabled' -or $MongoDB -eq 'enabled') {
    & "$PSScriptRoot\install-databases.ps1" -Cluster $Name -Postgres $Postgres -MongoDB $MongoDB
}
if ($Redis -eq 'enabled' -or $Kafka -eq 'enabled' -or $MinIO -eq 'enabled' -or $Kyverno -eq 'enabled') {
    & "$PSScriptRoot\install-extras.ps1" -Cluster $Name -Redis $Redis -Kafka $Kafka -MinIO $MinIO -Kyverno $Kyverno
}
...
& "$PSScriptRoot\smoke-test.ps1" -Cluster $Name
Write-Host "`nCluster '$Name' ready. HTTP: http://localhost:8080 HTTPS: https://localhost:8443"
