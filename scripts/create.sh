...
if [[ "$INGRESS" != none ]];then
  kubectl --context "$CTX" label node "$NAME-control-plane" ingress-ready=true --overwrite
  if [[ "$INGRESS" == nginx ]];then
    helm repo add ingress-nginx https://kubernetes.github.io/ingress-nginx --force-update;helm repo update
    helm upgrade --install ingress-nginx ingress-nginx/ingress-nginx --kube-context "$CTX" -n ingress-nginx --create-namespace --set controller.kind=DaemonSet --set controller.service.type=ClusterIP --set controller.hostPort.enabled=true --set-string controller.nodeSelector.ingress-ready=true --set controller.tolerations[0].key=node-role.kubernetes.io/control-plane --set controller.tolerations[0].operator=Exists --set controller.tolerations[0].effect=NoSchedule
    kubectl --context "$CTX" -n ingress-nginx rollout status ds/ingress-nginx-controller --timeout=300s
  else
